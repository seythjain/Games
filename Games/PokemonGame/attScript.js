...
var power = Math.floor(Math.random() * mode) + 1;
//var pp = [4, 5, 6, 2, 4, 5, 6, 2, 4, 5, 6, 2, 4, 5, 6, 2, 4, 5, 6, 2,];
...
function battle(move) {
    ...
    var multiMon = ['pikachu', 'bulbasaur', 'squirtle', 'charmander']
    var pokemon = document.getElementById('bigPoke').src;
//  When making power points and power, make power not random
    var power = Math.floor(Math.random() * mode) + 1;
    var ppInit = [4, 5, 6, 2, 4, 5, 6, 2, 4, 5, 6, 2, 4, 5, 6, 2];
    var beforeHp = oppHP;

    for (let x = 0; x < multiMon.length; x++) {

        // for (let i = 0; i < 4; i++) {
        //     document.getElementById('pp' + (i + 1)).innerHTML = pp[i] + '/' + ppInit[i];

        // }


        if (pokemon.includes(multiMon[x] + 'HomePage.gif') == true || pokemon.includes(multiMon[x] + 'Attack.gif') == true) {
            // your attack        

            //             for (let i = 0; i < moves.length; i++) {
            //                 // if (pp[i] == 0) {
            //                 //     document.getElementById('pp' + (i + 1))
            //                 // }
            //                 if (moves[i] == move) {
            //                     power = movePower[i];
            // //                    pp[i] - 1
            ...
            document.getElementById('bigPoke').src = multiMon[x] + 'Attack.gif';
            var random = power;
            oppHP -= random;
            ...
            document.getElementById('oppHP').innerHTML = "Opponent Hp: " + oppHP;
            setTimeout(() => { document.getElementById('bigPoke').src = multiMon[x] + 'HomePage.gif'; }, 1000);
            ...
            document.getElementById('multimove4').disabled = true;
            document.getElementById('potion').disabled = true;
            document.getElementById('commentator').innerHTML = multiMon[x] + ' used ' + move;
            setTimeout(() => {document.getElementById('commentator').innerHTML = multiMon[x] + ' did ' + damage + ' damage.'; }, 2000);
            // makes sure u can see ur damage done 2 opponent
